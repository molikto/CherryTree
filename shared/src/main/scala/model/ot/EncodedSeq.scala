...
import operation.EncodedSeq._

object EncodedSeq extends Ot[data.EncodedSeq, operation.EncodedSeq, conflict.EncodedSeq] {


  type RebaseResult = Rebased[conflict.EncodedSeq, (Seq[operation.EncodedSeq], Seq[operation.EncodedSeq])]



  override def rebase(winner: operation.EncodedSeq, loser: operation.EncodedSeq): RebaseResult = {
    def addDelete(add: Insert, delete: Delete, deleteConflict: => conflict.EncodedSeq): RebaseResult = {
      val wat = add.at
      val wc = add.unicode
    ...
    def deleteReplaceAtomic(delete: Delete, replace: ReplaceAtomic, deleteConflict: => conflict.EncodedSeq): RebaseResult = {
    ...
    def deleteSurround(d: Delete, s: Surround, deleteConflict: => conflict.EncodedSeq): RebaseResult = {
    ...
    def overlapSurround(s: Surround, l: Surround, sWins: Boolean): RebaseResult = {
      ...
      val si = order.indexOf(s.left.singleSpecial)
      val li = order.indexOf(l.left.singleSpecial)
      ...
      case (w@Surround(wr, ws, we, wid), l@Surround(lr, ls, le, lid)) =>
        if (wid && lid && ws == ls && we == le && wr.overlap(lr)) { // [][] is not included now, who knows
          ...
          if (wr == lr) {
            ...
            val order = SpecialChar.breakOthersOrderedUnicode
            val wi = order.indexOf(ws.singleSpecial)
            val li = order.indexOf(ls.singleSpecial)
            if (wi > li) {
              wrapLInW()
            } else {
              wrapWInL()

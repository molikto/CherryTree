...
class NodeMove(settings: Settings) extends CommandCategory(settings,"node: move") {
  ...
  new IndentCommand {
    ...
    override def targetTo(a: DocState, node: range.Node): Option[cursor.Node] = {
      val mover = a.mover()
      mover.previous(node.start).filter(n => a.node(n).canHaveChilds).map(a => a :+ mover.size(a))
    }
  }
  new MoveCommand {
  ...
  new TextualCommand {
    ...
    override def available(a: DocState): Boolean = a.isSingle && {
      val asSingle = a.asSingle
      asSingle != cursor.Node.root && a.childIsLists(asSingle) == a.childIsLists(model.cursor.Node.parent(asSingle))
    }
    override def action(a: DocState, commandState: CommandInterface, count: Int): DocTransaction = {
      val mm = a.asSingle
      val n = a.node(mm)
